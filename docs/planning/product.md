...
- **Provider-agnostic** (Cursor/Copilot/ChatGPT/Gemini) is the TAM/north-star slide. **Lead Claude-Code-native** to the Anthropic reps — it's the only zero-friction ingestion path (`SessionStart` hook + local JSONL), not a compromise.
