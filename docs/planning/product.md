...
- **Broad provider-agnostic** (Cursor/Copilot/ChatGPT/Gemini) stays the TAM/north-star slide, and **Topic Analyst/Coach/nudge parity across providers stays roadmap.** _~~Exception, pulled into Saturday scope (a Codex-native partner joined):~~ **Reverted 2026-06-21 — see ADR-0004 and the Log entry below: Codex was dropped entirely.**_ ~~two-provider **ingestion** (Claude Code + Codex) via a `ProviderAdapter` interface + provider-labeled provenance, cheap to build/test on the partner's own real sessions. The Codex adapter is the vision-beat proof the abstraction is real;~~ the **spine stays Claude-Code-native** — Topic Analyst, hero loop, `SessionStart` nudge, and Coach run on Claude only. The `ProviderAdapter` interface + provider-labeled provenance abstraction is retained (now Claude Code + git), but Codex is no longer a shipped provider. See `docs/adr/0004-drop-codex-single-provider-selectable-coach-model.md`, ADR-0001 (superseded), and ADR-0002.
...
### 2026-06-21 (Codex dropped → single-provider Claude-native + selectable coach model)
Reversed the dual-provider bet from 2026-06-20. **Codex is dropped entirely** — the `CodexAdapter`, the half-built `CodexCoach`, the Codex exercise-generation fallback, and every Codex UI mention (enrollment log-sources, coach selector, provenance chip) are removed; `codex` is no longer in `SUPPORTED_PROVIDERS` and ingestion refuses it (HTTP 400). The provider-blind `ProviderAdapter` abstraction is **kept** — shipped adapters are now **Claude Code** (conversation provenance) + **git** (code reality) — so a future provider can be added without touching the spine. Separately, the Coach gains a **model selector**: it stays Claude-only with all tools denied, but the maintainer chooses which Claude model answers (`haiku | sonnet | opus`, default `sonnet`, env `LEDGER_COACH_MODEL`), threaded `create_coach(model)` → `ClaudeCoach.model_id` → `claude -p --model …` (mirrors `ClaudeAnalyst`). `--model` is orthogonal to `--disallowedTools`, so withholding is never weakened. Recorded in **ADR-0004**; ADR-0001 marked superseded. "Support both providers" no longer holds.
