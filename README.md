...
- **Selects** — the **Topic Analyst** (a scoped Claude Code harness with read-only `Read`/`Grep`/`Glob`) investigates your repository and its real Claude session traces to construct an ordered **worklist** of durable maintenance obligations. Deterministic code then verifies every citation, hashes the evidence, and persists it. No item exists without exact code grounding.
- **Surfaces just-in-time** — a `SessionStart` hook reads your `cwd`/branch and tells you about debt *on the path you're about to touch*, with deep links into the app.
- **Verifies (Debug-to-Own)** — starting a **Check** spins up a real temp-dir sandbox of your committed code, injects a small blast-controlled semantic defect targeted at the flagged decision, and makes you diagnose and repair it until the test suite goes green. The signal is *struggle* (time, attempts, coach use) — never the binary solve.
- **Coaches without leaking** — the **Coach** runs on your own Claude Code CLI (`claude -p`, **all tools denied**), so it explains concepts, asks diagnostic questions, and suggests experiments but *architecturally cannot* hand you the patch. You pick which Claude model answers (`haiku | sonnet | opus`, default `sonnet`).
- **Remembers** — topics, evidence, attempts, and reflections persist in a local SQLite ledger across sessions and code changes; a tracked decision that gets modified again (still untrailed, previously fumbled) re-surfaces to the top.

Ledger **recommends checks; it never asserts that you lack understanding** and never grades your code. The headline is *"this deserves a check,"* inferred from silence — only the check result turns a candidate into confirmed debt.
...
Provenance model: **code is truth** (what you exercise), **docs/ADRs/comments are the trail** (whether the *why* was captured), **the transcript is the receipt** (whether Claude was involved). The transcript is deliberately kept *out* of validation — diff↔session linking is fuzzy — and is stored as provenance display only.
...
- **Claude Code CLI** on your `PATH` — Ledger's only Claude dependency. The Topic Analyst and Coach shell out to `claude -p`; you ride your existing Claude Code auth, so there's no API key to manage and no per-token cost.
...
> The seeded demo ships a fully-grounded **tenant-isolation** hero topic and its Debug-to-Own check, so you can walk the full loop (worklist → topic card → sandbox → coach → repair → reflection → persisted history) with no live extraction. Curated components are disclosed as curated.

### Running against a real repository

Point the analyst at one of your own Git repos to discover its worklist. Extraction ingests that repo's *real* `~/.claude` transcripts as Agent-trace evidence.
